...
// Get live price for an asset symbol from prices.json
function getLivePrice(symbol) {
  if (!state.prices || !state.prices.prices) return null;
  return state.prices.prices[symbol] || null;
}

// Format price with appropriate decimals based on value
function fmtLivePrice(price, decimals) {
...
function drawSparklineChart(canvasId, weeks) {
  ...
  // Add live data point if available (use last spec value as placeholder)
  if (state.prices && state.prices.prices) {
    // Sparkline shows positioning trend, not price - no live point needed
    // but we could extend the trend line visually
  }
...
function drawChart(asset) {
  ...
  // Append live price as the latest data point if available
  const live = getLivePrice(asset.symbol);
  if (live && live.price) {
    const today = new Date().toISOString().split('T')[0];
    labels.push(today + ' (LIVE)');
    // Positioning data stays the same (no new COT report yet)
    specNet.push(specNet[specNet.length - 1]);
    hedgeNet.push(hedgeNet[hedgeNet.length - 1]);
    prices.push(live.price);
  }
  ...
  if (asset.hasPriceData || (live && live.price)) {
    // Create point radius array - last point (live) gets a bigger dot
    const pointRadii = new Array(prices.length).fill(0);
    if (live && live.price) pointRadii[pointRadii.length - 1] = 6;
    ...
    options: {
      ...
      scales: {
        ...
        yPrice: {
          type: 'linear',
          position: 'right',
          display: (asset.hasPriceData || (live && live.price)),
...
function assetCard(asset) {
  ...
  // Try to get live price, fall back to COT weekly price
  const live = getLivePrice(asset.symbol);
  const hasLive = live && live.price;
  const livePrice = hasLive ? live.price : latest.price;
  const liveChg = hasLive ? live.changePct : (latest.priceChg ? (latest.priceChg / latest.price * 100) : 0);
  const priceFormatted = livePrice ? fmtPrice(livePrice) : '--';
  const priceChgClass = hasLive ? (liveChg >= 0 ? 'pos' : 'neg') : dirClass(latest.priceChg);
  const priceChgFormatted = hasLive ? fmtPctVal(liveChg) : (latest.priceChg ? fmtSigned2(latest.priceChg) : '');
  const liveBadge = hasLive ? '<span class="badge badge-live">LIVE</span>' : '<span class="badge badge-info">1W</span>';
      ...
      <div class="news-card">
        <div class="news-header">
          <div class="news-meta">
            ...
            <span>•</span>
            <span class="news-time">${formatTimeAgo(item.pubDate)}</span><span class="news-date">${new Date(item.pubDate).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})} ${new Date(item.pubDate).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",hour12:false})}</span>
          </div>
...
function renderDetail() {
  ...
  const specLongPct = (latest.specLong / specTotal) * 100;
  const specShortPct = (latest.specShort / specTotal) * 100;
  drawGaugeChart('detail-gauge', specLongPct, specShortPct);
  drawOIChart(asset);
  drawPctChart(asset);
...
function detailTemplate(asset, latest, ins, ext) {
  ...
  return `
    <div class="detail-header">
      ...
      <div class="detail-title-row">
        ...
        <div style="display: flex; gap: 8px; align-items: center;">
          ${(() => { const lp = getLivePrice(asset.symbol); if (lp && lp.price) { const cls = lp.changePct >= 0 ? 'pos' : 'neg'; return '<span class="badge badge-live">● LIVE $' + fmtLivePrice(lp.price) + ' (' + fmtPctVal(lp.changePct) + ')</span>'; } return ''; })()}
    ...
    <div class="detail-kpi-grid">
      <div class="kpi-card">
        <span class="kpi-label">${(() => { const lp = getLivePrice(asset.symbol); return lp && lp.price ? 'Live Price' : 'Latest Price (1W)'; })()}</span>
        <span class="kpi-value num">${(() => { const lp = getLivePrice(asset.symbol); return lp && lp.price ? '$' + fmtLivePrice(lp.price) : (latest.price ? '$' + fmtPrice(latest.price) : '--'); })()}</span>
        <span class="kpi-sub num">${(() => { const lp = getLivePrice(asset.symbol); if (lp && lp.changePct !== undefined) { const cls = lp.changePct >= 0 ? 'pos' : 'neg'; return '<span class="' + cls + '">' + fmtPctVal(lp.changePct) + '</span> today'; } return '<span class="' + dirClass(latest.priceChg) + '">' + (latest.priceChg ? fmtSigned2(latest.priceChg) : '') + '</span> 1W'; })()}</span>
      </div>
    ...
    <div class="detail-sentiment-section">
      <h2 style="font-size: 1.1rem; margin-bottom: 16px;">Market Sentiment & Positioning</h2>
      <div class="detail-sentiment-grid">
        <div class="chart-card" style="text-align: center;">
          <div class="chart-card-header"><span class="chart-card-title">Long vs Short Positioning</span></div>
          <div class="gauge-container" style="height: 180px; margin: 16px auto; max-width: 300px;">
            <canvas id="detail-gauge"></canvas>
            <div class="gauge-center-text">
              <div class="gauge-center-val ${dirClass(latest.specNet)}">${ins.rangePct.toFixed(0)}%</div>
              <div class="gauge-center-lbl">Percentile</div>
            </div>
          ...
          <div style="display: flex; justify-content: center; gap: 16px; margin-bottom: 12px;">
            <span style="color: var(--color-bullish);" class="num">Long: ${specLongPct.toFixed(1)}%</span>
            <span style="color: var(--color-bearish);" class="num">Short: ${specShortPct.toFixed(1)}%</span>
          </div>
          <span class="badge ${ins.rangePct >= 70 ? 'badge-bullish' : (ins.rangePct <= 30 ? 'badge-bearish' : 'badge-neutral')}">${ext}</span>
        </div>

        <div class="chart-card">
          <div class="chart-card-header"><span class="chart-card-title">52-Week Positioning Range</span></div>
          <div style="padding: 20px;">
            ${rangeGaugeSVG(ins.rangePct)}
            <div style="margin-top: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 0.85rem;">
              <div><span class="kpi-label">52W Min Net:</span> <span class="num">${fmtSigned(ins.minNet)}</span></div>
              <div><span class="kpi-label">52W Max Net:</span> <span class="num">${fmtSigned(ins.maxNet)}</span></div>
              <div><span class="kpi-label">4W Net Change:</span> <span class="num ${dirClass(ins.netChg4w)}">${fmtSigned(ins.netChg4w)}</span></div>
              <div><span class="kpi-label">4W Price Change:</span> <span class="num ${dirClass(ins.priceChg4w)}">${ins.priceChg4w ? fmtSigned2(ins.priceChg4w) : '-'}</span></div>
              <div><span class="kpi-label">Streak:</span> <span class="num">${ins.streak} weeks ${ins.streakDir === 'pos' ? '↑' : '↓'}</span></div>
              <div><span class="kpi-label">Total OI:</span> <span class="num">${fmtInt(latest.totalOI)}</span></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style="margin-bottom: 32px;">
      <h2 style="font-size: 1.1rem; margin-bottom: 16px;">Related News & Analysis</h2>
...
function renderAssetRelatedNews(symbol) {
  ...
  // Find ALL news items tagged with this asset
  ...
  // Aggregate sentiment from all related news
  let bullishCount = 0, bearishCount = 0, neutralCount = 0;
  items.forEach(item => {
    const assetData = item.analysis.assets.find(a => a.symbol.toUpperCase() === symbol.toUpperCase());
    if (assetData) {
      if (assetData.direction === 'bullish') bullishCount++;
      else if (assetData.direction === 'bearish') bearishCount++;
      else neutralCount++;
    }
  });

  // Build sentiment summary header
  const total = items.length;
  const bullPct = Math.round((bullishCount / total) * 100);
  const bearPct = Math.round((bearishCount / total) * 100);
  const overallDir = bullishCount > bearishCount ? 'bullish' : (bearishCount > bullishCount ? 'bearish' : 'neutral');
  const overallClass = overallDir === 'bullish' ? 'badge-bullish' : (overallDir === 'bearish' ? 'badge-bearish' : 'badge-neutral');

  let summaryHtml = `
    <div class="news-sentiment-summary">
      <h3 style="font-size: 1.1rem; margin-bottom: 12px;">Related News & Analysis (${items.length} items)</h3>
      <div class="news-sentiment-bars">
        <div class="sentiment-bar-row">
          <span class="badge badge-bullish">Bullish: ${bullishCount} (${bullPct}%)</span>
          <span class="badge badge-neutral">Neutral: ${neutralCount} (${Math.round((neutralCount/total)*100)}%)</span>
          <span class="badge badge-bearish">Bearish: ${bearishCount} (${bearPct}%)</span>
        </div>
      </div>
    </div>
  `;

  // Render ALL related news with full analysis
  const newsHtml = items.map(item => {
    const assetData = item.analysis.assets.find(a => a.symbol.toUpperCase() === symbol.toUpperCase());
    if (!assetData) return '';

    const dir = assetData.direction;
    const conf = assetData.confidence;
    const badgeClass = dir === 'bullish' ? 'badge-bullish' : (dir === 'bearish' ? 'badge-bearish' : 'badge-neutral');
    const explanation = assetData.explanation || '';
    ...
    return `
      <div class="news-card">
        ...
        ${item.description ? `<p class="news-description">${item.description}</p>` : ''}
        <div class="ai-analysis-box">
          <div class="ai-box-title">AI ANALYSIS FOR ${symbol}</div>
          <div class="ai-badges-row">
            <span class="badge ${badgeClass}">${dir.toUpperCase()} ${conf}%</span>
          </div>
          <div class="ai-summary">${explanation}</div>
        </div>
      </div>
    `;
  }).join('');

  container.innerHTML = summaryHtml + newsHtml;
}
...
async function loadData() {
  try {
    const [cotRes, newsRes, calRes, pricesRes] = await Promise.allSettled([
    ...
    if (pricesRes.status === 'fulfilled') state.prices = pricesRes.value;

    // Normalize COT week order: build_data.py writes weeks oldest-first
    // (ascending), but every render function in this file assumes
    // weeks[0] is the LATEST week. Sort descending here, once, so the
    // rest of the app doesn't need touching.
    if (state.cot && state.cot.assets) {
      state.cot.assets.forEach(a => {
        if (a.weeks && a.weeks.length > 1) {
          a.weeks.sort((x, y) => (x.date < y.date ? 1 : (x.date > y.date ? -1 : 0)));
        }
      });
    }
